...
//   bindNestedEvents();
        $log = $('#tab_log_window');
        $log.attr({scrollTop: 100500 }); /// [scroll] todo!
//         debug('scrolling (_XTouchTab)');
};
...
function _XCloseOtherTabs (_contact) {
  ...
  _getTab(0);
        ...
        $log = $('#tab_log_window');
        $log.attr({scrollTop: 100500 }); /// [scroll] todo!
//         debug('scrolling (_XCloseOtherTabs)');
};
...
//  debug('XAddTab for \'' + _contact.nick + '\'');
  var title = _contact.nick + "<a href=# onclick=$('#chatTabs').flyTabs.XDelTab(CList.contacts['"+_contact.id+"']);> <img src=i/test2.bmp /></a>";
  var args = {
        caption: title,
...
**********************************************************************/
 ...
 function _bindEvents (parentThis, tabElement, currentSelector) {
    ...
    $(tabElement).click(function(){
        ...
        $log = $('#tab_log_window');
        $log.attr({scrollTop: 100500 }); /// [scroll] todo!
//         debug('scrolling ($(tabElement).click)');
